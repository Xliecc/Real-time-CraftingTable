...
**A real-time 3D preview of your crafting result — materials float above the workbench, and the crafted result hovers and slowly rotates above them.**

> A Fabric mod that adds a **real-time 3D preview** to the vanilla Crafting Table.
> Install on your client and the preview works — even on vanilla servers. Install on the server too for item persistence, real-time sharing and facing sync. *(Client and server are independent: either side may be installed alone.)*
...
## ❓ FAQ
- **Preview not showing**: Make sure you open a 3×3 Crafting Table screen (not the 2×2 inventory one), keep the workbench in view, and check `enabled` and `renderDistance`.
- **Config screen missing with Mod Menu**: Install [Cloth Config](https://modrinth.com/mod/cloth-config) as well (it is pulled in automatically by Mod Menu).
